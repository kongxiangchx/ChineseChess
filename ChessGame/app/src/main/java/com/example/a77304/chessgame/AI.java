...
public class AI {
    ...
    public static Move getBestMove(int[][] piece, boolean isRedGo, int depth,int startZobristKey,long startZobristKeyCheck){
        ...
        for(int i=2;i<=depth;i+=2){
            goodMove=getGoodMove(piece,isRedGo,i,startZobristKey,startZobristKeyCheck);
            if(i==2){
                tmpMove=goodMove;
            }
        }
        Pos fromPos=goodMove.fromPos;
        Pos toPos=goodMove.toPos;
        int tmp=piece[toPos.y][toPos.x];
        piece[toPos.y][toPos.x] = piece[fromPos.y][fromPos.x];
        piece[fromPos.y][fromPos.x] = 0;
        if(Rule.isKingDanger(piece,isRedGo)==true){
            goodMove=tmpMove;
        }
            ...
            for (int j = 0; j <= 8; j++) {
                if ((isRedGo == false && pieceClone[i][j] >= 1 && pieceClone[i][j] <= 7) || (isRedGo == true && pieceClone[i][j] >= 8 && pieceClone[i][j] <= 14)) {
                    ...
                    while (it.hasNext()) {
                        ...
                        updateZobrist(pieceClone,fromPos,toPos,pieceClone[toPos.y][toPos.x],pieceID);
                        ...
                        if (value > alpha) {
                            ...
                            transformTable.saveTransformTable(tmpZobristKey,tmpZobristKeyCheck,maxDepth-depth,alpha);
                        }
                    }
                }
            }
    ...
    public static int PVS(int[][] piece, boolean isRedGo, int depth, int alpha, int beta, boolean isKingAlive) {
        boolean FoundPv = false;
        int value=transformTable.readTransformTable(ZobristKey,ZobristKeyCheck,maxDepth-depth);
        if(value!=INF){
            return value;
        }
        ...
        if (isKingAlive==false||depth <= 0) {
            ...
            transformTable.saveTransformTable(ZobristKey,ZobristKeyCheck,maxDepth-depth,value);
            return value;
        }
            ...
            for (int j = 0; j <= 8; j++) {
                if ((isRedGo == false && piece[i][j] >= 1 && piece[i][j] <= 7) || (isRedGo == true && piece[i][j] >= 8 && piece[i][j] <= 14)) {
                    ...
                    while (it.hasNext()) {
                        ...
                        if (value >= beta) {
                            transformTable.saveTransformTable(tmpZobristKey,tmpZobristKeyCheck,maxDepth-depth,beta);
                            return beta;
                        }
                        if (value > alpha) {
                            alpha = value;
